...
export function renderTrend(target, records) {
  ...
  target.innerHTML = svg(`<g class="grid">${grid}</g><polyline class="trend-line" points="${points}"/><g class="trend-points">${records.map((item, index) => `<circle cx="${x(index)}" cy="${y(item.weight)}" r="${index === records.length - 1 ? 5 : 2.5}"><title>${esc(item.date)} · ${item.weight.toFixed(2)} kg</title></circle>`).join('')}</g>${labels}`);
}
...
export function renderBars(target, values, { color = 'purple', formatter = value => `${value}g`, labels = [], yAxis = false } = {}) {
  ...
  const bars = values.map((value, index) => {
    ...
    return `<rect class="bar ${color} ${value < 0 ? 'negative' : ''}" x="${px}" y="${py}" width="${Math.max(4, width - 5)}" height="${height}" rx="3"><title>${esc(label)} · ${formatter(value)}</title></rect>`;
  }).join('');
